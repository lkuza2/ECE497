...
function LEDclick(i, j) {
    ...
    // Toggle bit on display
 		if ($('#id'+i+'_'+j).hasClass('green_box')) {
			$('#id'+i+'_'+j).removeClass('green_box');
			green[i] ^= 0x1<<j;
			socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i, 
			     disp: '0x'+green[i].toString(16)});
			$('#id'+i+'_'+j).addClass('red_box');	
			red[i] ^= 0x1<<j;
			socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i+1, 
			     disp: '0x'+red[i].toString(16)});

		} else if($('#id'+i+'_'+j).hasClass('red_box')) {
			$('#id'+i+'_'+j).removeClass('red_box');
		    red[i] ^= 0x1<<j;
			socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i+1, 
			     disp: '0x'+red[i].toString(16)});
			$('#id'+i+'_'+j).addClass('orange_box');
			red[i] ^= 0x1<<j;
			green[i] ^= 0x1<<j;
			socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i, 
			     disp: '0x'+green[i].toString(16)});
		...
		} else if($('#id'+i+'_'+j).hasClass('orange_box')) {
			$('#id'+i+'_'+j).removeClass('orange_box');
			red[i] ^= 0x1<<j;
			green[i] ^= 0x1<<j;
			socket.emit('i2cset', {i2cNum: i2cNum, i: 2*i, 
			     disp: '0x'+green[i].toString(16)});
